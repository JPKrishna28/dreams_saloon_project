...
router.post('/staff/login', [
    ...
    try {
        ...
        // Find staff member with login credentials
        const staff = await Employee.findOne({ 
            'loginCredentials.username': username,
            'loginCredentials.isLoginEnabled': true,
            isActive: true 
        });
        ...
        const isPasswordValid = await bcrypt.compare(password, staff.loginCredentials.password);
        if (!isPasswordValid) {
            return res.status(401).json({
                success: false,
                message: 'Invalid credentials'
        ...
        // Update last login
        staff.loginCredentials.lastLogin = new Date();
        await staff.save();
        ...
        const token = jwt.sign(
            { 
                userId: staff._id, 
                username: staff.loginCredentials.username, 
...
router.get('/staff/profile', authenticateToken, async (req, res) => {
    try {
        ...
        const staff = await Employee.findById(req.user.userId)
            .select('-loginCredentials.password -__v');
